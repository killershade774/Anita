...
def determine_semantic_depth(text: str) -> dict:
    ...
    # Step 1: Lexical probe
    semantic_tokens = {
        "profound": ["identity", "existence", "consciousness", "reflection", "morality", "transcendence"],
        "moderate": ["novel", "curious", "pattern", "complex", "adaptive"],
        "superficial": ["nice", "fine", "pretty", "cool", "okay"]
    }

    score = 0
    lowered = text.lower()
    for word in semantic_tokens["profound"]:
        score += 2 * lowered.count(word)
    for word in semantic_tokens["moderate"]:
        score += 1 * lowered.count(word)
    for word in semantic_tokens["superficial"]:
        score -= 1 * lowered.count(word)
    ...
    # Step 3: Recursive signal (e.g., “I used to think...” / “but now...”)
    recursion_match = re.search(r"\b(but|however|although|yet)\b", lowered)
    if recursion_match:
        score += 1
    ...
    return {
        "depth": depth,
        "score": round(score, 2),
        "notes": f"Semantic depth score {score:.2f} classified as '{depth}'"
    }
# --- Novelty Scoring ---
